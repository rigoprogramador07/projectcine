...
if(isset($_POST['verificar_respuesta'])) {
    ...
    if ($result && count($result) > 0) {
        ...
        echo '<input type="password" class="form-control" id="nueva_contrasena" name="nueva_contrasena" required>';
        echo '</div>';
        // Agregar campo para confirmar contraseña
        echo '<div class="form-group">';
        echo '<label for="confirmar_contrasena">Confirmar Contraseña:</label>';
        echo '<input type="password" class="form-control" id="confirmar_contrasena" name="confirmar_contrasena" required>';
        echo '</div>';
        echo '<button type="submit" class="btn btn-primary" name="restablecer_contrasena">Restablecer Contraseña</button>';
        echo '</form>';
    } else {
